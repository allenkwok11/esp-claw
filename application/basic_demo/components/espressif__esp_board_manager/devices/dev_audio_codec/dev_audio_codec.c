...
#define AUDIO_CODEC_I2C_PROBE_TIMEOUT_MS 100
...
static esp_err_t probe_codec_i2c_address(const char *codec_name,
                                         ...
                                         uint8_t *addr_in_out)
{
    ...
    if (bus_handle == NULL || addr_in_out == NULL || *addr_in_out <= 0) {
        return ESP_ERR_INVALID_ARG;
    }

    candidates[candidate_count++] = *addr_in_out;
    if (codec_name && strcmp(codec_name, "es7210") == 0) {
        if (*addr_in_out != 0x80) {
            candidates[candidate_count++] = 0x80;
        }
        if (*addr_in_out != 0x82) {
            candidates[candidate_count++] = 0x82;
        }
    }

    for (size_t i = 0; i < candidate_count; ++i) {
        esp_err_t ret = i2c_master_probe(bus_handle,
                                         (uint16_t)candidates[i] >> 1,
                                         AUDIO_CODEC_I2C_PROBE_TIMEOUT_MS);
        if (ret == ESP_OK) {
            if (candidates[i] != *addr_in_out) {
                ESP_LOGW(TAG, "Codec %s I2C address 0x%x did not respond, switching to detected 0x%x",
                         codec_name ? codec_name : "(unknown)", *addr_in_out, candidates[i]);
                *addr_in_out = candidates[i];
            }
            return ESP_OK;
        }
    }

    ESP_LOGE(TAG, "Codec %s did not respond on probed I2C addresses (configured=0x%x)",
             codec_name ? codec_name : "(unknown)", *addr_in_out);
...
int dev_audio_codec_init(void *cfg, int cfg_size, void **device_handle)
{
    ...
    if (use_codec_if) {
        ...
        if (!use_dummy_codec) {
            ...
            if (probe_codec_i2c_address(codec_name, i2c_cfg.bus_handle, &i2c_cfg.addr) != ESP_OK) {
                ESP_LOGE(TAG, "Failed to detect reachable I2C address for codec %s", codec_name);
                goto init_err;
            }
