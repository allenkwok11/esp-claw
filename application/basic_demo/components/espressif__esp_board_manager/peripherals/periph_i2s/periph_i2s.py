...
def format_data_bit_width(value) -> str:
    """Normalize YAML data bit width into an ESP-IDF enum string.

    Accepts integers like 16/24/32, numeric strings, or full enum names such as
    I2S_DATA_BIT_WIDTH_32BIT.
    """
    ...
    if isinstance(value, int):
        return f'I2S_DATA_BIT_WIDTH_{value}BIT'

    value_str = str(value).strip()
    if re.fullmatch(r'\d+', value_str):
        return f'I2S_DATA_BIT_WIDTH_{value_str}BIT'

    if value_str.startswith('I2S_DATA_BIT_WIDTH_'):
        if not validate_enum_value(value_str, 'data_bit_width'):
            raise ValueError(
                f"Invalid data_bit_width value '{value_str}'. "
                'Please use a valid ESP-IDF enum or numeric bit width.'
            )
        return value_str

    candidate = f'I2S_DATA_BIT_WIDTH_{value_str.upper()}'
    if candidate.endswith('BIT') and validate_enum_value(candidate, 'data_bit_width'):
        return candidate

    raise ValueError(
        f"Invalid data_bit_width value '{value}'. "
        'Use 8/16/24/32 or a full enum like I2S_DATA_BIT_WIDTH_32BIT.'
    )
...
def build_std_slot_config(cfg: dict, hw_version: int) -> dict:
    ...
    """
    slot_cfg = {
        'data_bit_width': format_data_bit_width(cfg.get('data_bit_width', 16)),
